...
func goOnEncodedVideoFrame(observer unsafe.Pointer, uid C.uint32_t, imageBuffer *C.uint8_t, length C.size_t,
	...
	receiver := agoraService.remoteEncodedVideoReceivers[observer]
	agoraService.remoteVideoRWMutex.RUnlock()
	if receiver == nil || receiver.receiver == nil || receiver.receiver.OnEncodedVideoFrame == nil {
		return C.int(0)
	}
	...
	if receiver.receiver.OnEncodedVideoFrame(receiver.receiver, goUid, goImageBuffer, goFrameInfo) {
		return C.int(1)
	}
	return C.int(0)
}
