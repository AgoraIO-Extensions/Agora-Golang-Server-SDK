...
// RemoteAudioFilterProcessor manages remote audio filtering
type RemoteAudioFilterProcessor struct {
...
func NewRemoteAudioFilterProcessor() *RemoteAudioFilterProcessor {
	return &RemoteAudioFilterProcessor{
		isRunning: true,
	}
}

// Initialize initializes the RTC connections (sender and receiver)
func (p *RemoteAudioFilterProcessor) Initialize(appid, channelId, senderUid, receiverUid string) error {
	p.channelId = channelId
	p.senderUid = senderUid
	p.receiverUid = receiverUid

	rootDir := "/Volumes/ZR/Agora/SERVER/SDK/Agora-Golang-Server-SDK/go_sdk/examples/apm_filter_remote"
	...
	if ret != 0 {
		fmt.Printf("Failed to enable audio_processing_remote_playback extension, error: %d\n", ret)
	} else {
	...
	if err := p.createSenderConnection(); err != nil {
		return err
	}
...
func (p *RemoteAudioFilterProcessor) createSenderConnection() error {
...
func (p *RemoteAudioFilterProcessor) createReceiverConnection() error {
	...
	if ret != 0 {
		fmt.Printf("Failed to set audio frame parameters, error: %d\n", ret)
	}
...
func (p *RemoteAudioFilterProcessor) setupReceiverObserver() {
	observer := &agoraservice.LocalUserObserver{
		OnUserAudioTrackSubscribed: func(localUser *agoraservice.LocalUser, uid string, remoteAudioTrack *agoraservice.RemoteAudioTrack) {
			...
			// Load AINS resource
			ret := remoteAudioTrack.SetFilterProperty("audio_processing_remote_playback", "apm_load_resource", "ains", 2)
			if ret != 0 {
				fmt.Printf("Failed to set apm_load_resource, error: %d\n", ret)
			}

			// Set APM configuration
			configJSON := "{\"aec\":{\"split_srate_for_48k\":16000},\"ans\":{\"enabled\":true},\"sf_st_cfg\":{\"enabled\":true,\"ainsModelPref\":10},\"sf_ext_cfg\":{\"nsngAlgRoute\":12,\"nsngPredefAgg\":11}}"
			ret = remoteAudioTrack.SetFilterProperty("audio_processing_remote_playback", "apm_config", configJSON, 2)
			if ret != 0 {
				fmt.Printf("Failed to set apm_config, error: %d\n", ret)
				return
			}
			fmt.Println("APM config set")

			// Enable dump
			ret = remoteAudioTrack.SetFilterProperty("audio_processing_remote_playback", "apm_dump", "true", 2)
			if ret != 0 {
...
func (p *RemoteAudioFilterProcessor) SendAudioData(pcmFilePath string, sampleRate, channels int) {
...
func (p *RemoteAudioFilterProcessor) Run(pcmFilePath string, sampleRate, channels, duration int) {
...
func (p *RemoteAudioFilterProcessor) Cleanup() {
	fmt.Println("\n=== Cleanup resources ===")
...
var globalProcessor *RemoteAudioFilterProcessor
...
func main() {
	...
	pcmFilePath := "/Volumes/ZR/Agora/SERVER/SDK/Agora-Golang-Server-SDK/go_sdk/examples/apm_filter_remote/noise.pcm"
	...
	// Create processor
	processor := NewRemoteAudioFilterProcessor()
