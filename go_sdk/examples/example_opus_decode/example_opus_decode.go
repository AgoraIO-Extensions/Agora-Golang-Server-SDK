...
func main() {
	...
	*bStop = false
	...
	println("Start example_opus_decoder\nusage:\n	./example_opus_decode <appid> <channel_name>\n	press ctrl+c to exit\n")
	...
	opusDecoder := agoraservice.NewOpusDecoder(1, 16000, 1)
	if opusDecoder == nil {
		fmt.Println("Failed to create opus decoder")
		return
	}
	defer opusDecoder.Release()
	...
	audioObserver := &agoraservice.AudioFrameObserver{
		OnPlaybackAudioFrameBeforeMixing: func(localUser *agoraservice.LocalUser, channelId string, userId string, frame *agoraservice.AudioFrame, vadResultState agoraservice.VadState, vadResultFrame *agoraservice.AudioFrame) bool {
			// do something
			fmt.Printf("Playback audio frame before mixing, from userId %s, audioModel: %d\n", userId, audioModel)
			/*
			if audioModel == 1 {
